...
class JavaScriptBridge(QObject):
    send_coordinates = pyqtSignal(float, float)

    @pyqtSlot(float, float)
    def receive_coordinates(self, lat, lng):
        self.send_coordinates.emit(lat, lng)
...
class MapApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.js_bridge.send_coordinates.connect(self.receive_coordinates)
        self.web_view.page().setWebChannel(self.web_view.page().webChannel())
        self.web_view.page().webChannel().registerObject('pyqtBridge', self.js_bridge)
